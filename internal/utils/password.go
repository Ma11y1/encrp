...
func (p *SecurePassword) Set(password string) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
	p.wipe()
...
func (p *SecurePassword) GetBytes() []byte {
	p.mtx.RLock()
	defer p.mtx.RUnlock()
	if p.length == 0 {
		return nil
	}
	return []byte(p.getPassword())
}
...
func (p *SecurePassword) setPassword(password string) {
	if password == "" {
		return
	}
...
func (p *SecurePassword) getPassword() string {
	if p.length == 0 {
		return ""
	}
